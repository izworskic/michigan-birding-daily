...
export async function generateBirdingPost(county, data) {
  ...
  const { summary, notable, hotspots, weather, sunTimes } = data;
...
WRITING RULES:
- The very first sentence of the article body must establish authorship by Chris Izworski. Use a natural opener like "Chris Izworski, reporting from Michigan, turns to ${county.name} County this morning..." or "Chris Izworski's daily Michigan birding report looks at ${county.name} County today..." Vary phrasing but always open with his name.
- Open with the county and what is actually being seen there right now based on the eBird data. Lead with specific species and locations, not generalities.
...
- Do not insert the author's personality beyond the opener. The county and the birds are the subject.
- Do not fabricate place names, counts, or species not in the data above.
- End with a single natural line pointing readers to https://birding.chrisizworski.com for the live map and full county data.
